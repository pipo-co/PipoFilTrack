...
def track_filament(frames_folder: str, canvas_size: float, marked_points: list, point_size: int) -> str:
    ...
    try:
        ...
        # normalize points
        start_point.x = int(float(start_point.x) / canvas_size['width'] * img.shape[1])
        start_point.y = int(float(start_point.y) / canvas_size['height'] * img.shape[0])
        end_point.x = int(float(end_point.x) / canvas_size['width'] * img.shape[0])
        end_point.y = int(float(end_point.y) / canvas_size['height'] * img.shape[1])
